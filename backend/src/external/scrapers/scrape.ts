...
export class Scrape {
  ...
  public static async fetchWithHeaders(url: string, headers: Record<string, string>): Promise<AxiosResponse<any, any>> {
    try {
      const response = await axios({
        url,
        method: "GET",
      ...
      if (response.status === 301 || response.status === 302) {
        const redirectUrl = response.headers.location;
        if (redirectUrl) {
          return await this.fetchWithHeaders(redirectUrl, headers);
        }
      }
  ...
  public static async fetchWithHeadersJinaAI(url: string, headers: Record<string, string>): Promise<AxiosResponse<any, any>> {
    try {
      const response = await axios({
        url: `https://r.jina.ai/${url}`,
        method: "GET",
      ...
      if (response.status === 301 || response.status === 302) {
        const redirectUrl = response.headers.location;
        if (redirectUrl) {
          return await this.fetchWithHeadersJinaAI(redirectUrl, headers);
        }
      }
  ...
  public static async apiParser<R, T>(
    ...
    try {
      ...
      let serial = response.data.data.html.match(/<pre.*">(.*[\w\W]+)<\/pre>/im)[1] || "{}";
