<script lang="ts">
  ...
  import { mangaProvider as mangaProviderStore, animeProvider as animeProviderStore } from "$/stores/providerStore";
  ...
  let animeProviderSelect: string = "otakudesu";
  let mangaProviderSelect: string = "komiku";

  $: {
    btnSaveDisabled = $mangaProviderStore === mangaProviderSelect && $animeProviderStore === animeProviderSelect;
    if (!isClick) {
      animeProviderSelect = $animeProviderStore;
      mangaProviderSelect = $mangaProviderStore;
    }
  }
...
<section class="fixed top-30 right-0 block z-50">
  ...
  >
    <header class="flex items-center justify-between">
      <div>
        <h2 class="text-2xl font-semibold text-red-500">Provider Settings</h2>
      </div>
      <button
    ...
    <section>
      <label for="manga-provider" class="block mb-2 text-sm font-semibold text-white/80">
        Select Manga Provider
      </label>
      <select
        id="manga-provider"
        bind:value={mangaProviderSelect}
        class="w-full rounded-md bg-[#1a1a1a] text-white py-2 px-3 focus:outline-none focus:ring-2 focus:ring-red-500"
      >
        {#each mangaProvider as provider}
          <option value={provider.id}>{provider.name}</option>
        {/each}
      </select>
    </section>
