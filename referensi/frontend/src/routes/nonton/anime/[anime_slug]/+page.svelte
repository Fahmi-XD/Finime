<script lang="ts">
  ...
  import type { AnimeSlug } from "./+page";
  import DetailAnimeLayout from "$components/nonton/layouts/DetailAnimeLayout.svelte";
  ...
  onMount(async () => {
    try {
      const response = await fetchAnimeDetail(data.slug);
