...
<div class="bg-[hsl(var(--background))] text-[hsl(var(--foreground))] py-8">
  ...
  {:else if mangaList.length > 0}
    <div
      class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto"
    >
      {#each mangaList as manga, i (i)}
        <div
          class="bg-[hsl(var(--card))] rounded-lg shadow-sm border border-[hsl(var(--border))] overflow-hidden hover:shadow-md transition-all duration-200 flex flex-col h-full"
        >
          <img src={manga.thumb} alt={manga.title} class="w-full h-48 object-cover" />
          <div class="p-4 flex-1 min-h-[120px]">
            <h3
              class="text-lg font-semibold text-[hsl(var(--foreground))] line-clamp-2 mb-3 hover:text-[hsl(var(--primary))] transition-colors"
            >
              {manga.title}
            </h3>
            ...
            <div class="flex flex-wrap gap-2">
              <span
                class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-[hsl(var(--muted))] text-[hsl(var(--foreground))]"
              >
                {manga.type}
              </span>
            </div>
