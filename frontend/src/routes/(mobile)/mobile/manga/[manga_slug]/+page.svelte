...
<div class="p-4x mx-auto max-w-sm text-white" in:scale={{ duration: 200, start: 0.99 }}>
	...
	<div class="mt-3">
		...
		<div class="mt-3 flex items-center gap-1 text-xs font-semibold text-yellow-500">
	...
	<div class="my-4 flex items-center gap-2 text-xs">
		<a href="/mobile/manga/read/{(mangaDetail?.chapters?.[mangaDetail?.chapters?.length - 1]?.url || "")}" class="rounded-3xl bg-red-500 px-7 py-2">First Chapter 1</a>
		<a href="/mobile/manga/read/{(mangaDetail?.chapters?.[0]?.url || "")}" class="rounded-3xl border-2 border-red-500 bg-transparent px-7 py-2"
	...
	<div class="mt-5 flex flex-wrap justify-around gap-2 pb-10">
		{#each (mangaDetail?.chapters || []) as chapter, i (i)}	
			<a
				href="/mobile/manga/read/{(chapter?.url || "")}"
